...
public class PagingSpirit {
    ...
    public void fetchPage(@Nonnull List<TextComponentString> messages, int beginIndex, String guild) {
        ...
        end = (beginIndex - MAX_INDEX > 1 ? PAGE_BACK : PAGE_START).appendSibling(new TextComponentString(" --- " + beginIndex + "/" + size + " --- ").setStyle(new Style())).appendSibling((beginIndex + MAX_INDEX < size ? PAGE_NEXT : PAGE_END));
